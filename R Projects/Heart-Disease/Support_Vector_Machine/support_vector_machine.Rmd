...
categorical_columns <- list(
  ...
  fbs = list(levels = c(0, 1), labels = c("No", "Yes")),
  ...
  target = list(levels = c(1, 0), labels = c("Heart Disease Present", "No Heart Disease"))
)
...
```{r pair_plots, fig.width = 12, fig.height = 6, message=FALSE}
...
trainIdx = sample(1:nrow(Heart.df), size = 2/3 * nrow(Heart.df))

# Create a training dataset out of "Heart.df"
trainData = Heart.df[trainIdx, ]

# Create a testing dataset from the remaining dataset of "Heart.df"
testData  <- Heart.df[-trainIdx, ]
...
Build a support vector machine model with radial kernel and gamma = 10 and cost = 100.
```{r}
svm.model <- svm(target ~ ., data = trainData, kernel = "radial", gamma = 10, cost = 100)
summary(svm.model)
```
...
```{r}
tuned.results <- tune(svm, target ~ ., data = trainData, kernel = "radial", 
                     ranges = list(cost = c(1, 10, 50, 100), gamma = c(1, 3, 5)))
summary(tuned.results)
```

Determine best model parameters and print out the model results.
```{r}
...
Forecast `target` using the test dataset and the best model
```{r}
predictions <- predict(best_tuned.model, newdata = testData)
summary(predictions)
```

Get the true observations of `target` in the test dataset
```{r}
actuals <- testData$target
summary(actuals)
```

Compute the test error by constructing the confusion matrix
```{r}
